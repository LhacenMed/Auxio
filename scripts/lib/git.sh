...
# git::ensure_clean
# Aborts if there are uncommitted changes (staged or unstaged). Submodules are
# excluded: Auxio vendors media and taglib, whose build output routinely leaves
# them dirty without affecting what the pipeline builds from origin.
git::ensure_clean() {
